...
export function DepartmentCities({ departmentName, departmentCode, cities, category }: DepartmentCitiesProps) {
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {featuredCities.map((city) => (
                            <Link
                                key={city.slug}
                                href={category ? `/${category.slug}/${city.slug}` : `/annuaire`}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-2">
                        {listedCities.map((city) => (
                            <Link
                                ...
                                title={`${category?.name || 'Devis'} à ${city.name}`}
                            >
                                {city.name}
                            </Link>
                        ))}
